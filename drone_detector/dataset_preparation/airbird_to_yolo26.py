...
SOURCE_ROOT = Path(r"E:\Dataset_zip\AirBirds")
# contains images0..images10 and labels0..labels10

DEST_ROOT = Path(r"C:\Users\mehra\Downloads\Compressed\AirBird")
...
for i in range(11):
    img_dir = SOURCE_ROOT / f"images{i}"
    lbl_dir = SOURCE_ROOT / f"labels{i}"
    ...
    for img_path in img_dir.iterdir():
        ...
        if label_path.stat().st_size == 0:
            continue  # must contain at least one bird

        valid_pairs.append((img_path, label_path))

print(f"Found {len(valid_pairs)} valid bird images")

if len(valid_pairs) < TOTAL_IMAGES:
    raise RuntimeError("Not enough images with bird annotations")
...
# ================= EMPTY COPIED LABEL FILES =================

for split in ["train", "val", "test"]:
    labels_dir = DEST_ROOT / "labels" / split

    for label_file in labels_dir.glob("*.txt"):
        label_file.write_text("")

print("All copied annotation files have been emptied.")
