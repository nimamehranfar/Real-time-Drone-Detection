...
sys.path.append(str(_THIS_DIR.parent)) # Root
try:
    from detector.instrumentation_utils import VerificationLogger
except ImportError:
    pass
...
def main():
    ...
    logger = None
    try:
        logger = VerificationLogger(os.path.join(OUTPUT_ROOT, "nima_log.jsonl"))
    except Exception as e:
        print(f"Warning: could not init logger: {e}")
    ...
    while True:
        # Verification extras
        log_extras = {
            'infer_this_frame': False,
            'frame_id_at_start': frame_id,
            'troi_source': None,
            'troi_seed_box': None,
            'roi_params': None,
            'troi_detection_idx': None,
        }
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if is_infer:
            ...
            num_infer_frames += 1
            log_extras.update({
                'infer_this_frame': True,
                'frame_id_at_inference': frame_id,
                'num_yolo_calls': total_yolo_calls,
                'frame_size': (W, H),
            })
            ...
            if val_pre and len(eligible_prev) > 0:
                ...
                for pb in eligible_prev_sorted:
                    ...
                    total_yolo_calls += 1
                    if boxes_roi:
                        if log_extras['troi_source'] is None:
                            log_extras['troi_source'] = 'temporal'
                            log_extras['troi_seed_box'] = list(pb)
                            log_extras['roi_params'] = {'size': int(ROI_SIZE_LOCAL)}
                            try:
                                log_extras['troi_detection_idx'] = eligible_prev_sorted.index(pb)
                            except ValueError:
                                pass
        ...
        if logger:
            log_state = {**log_extras}
            # Manually add detections and state
            log_state.update({
                'warning_active': warning_active,
                'alert_active': alert_active,
                'detections': list(last_infer_final_boxes) if 'last_infer_final_boxes' in locals() else [],
                 # Buffers (deque to list)
                'win_warning_len': len(win_warning),
                'win_warning_sum': sum(win_warning),
                'win_alert_len': len(win_alert),
                'win_alert_sum': sum(win_alert),
                'warn_cooldown_left': warn_cooldown_left,
                'alert_cooldown_left': alert_cooldown_left,
                'final_troi': list(last_infer_guided_rois) if last_infer_guided_rois else None,
            })
            logger.log_frame(frame_id, log_state)
